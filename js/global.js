(() => {
  ...
  /* ============ Breathing pacer ambient tones (Nightstand, etc.) ============ */
  // The .pacer-static circle is a pure CSS animation (reset-breathe-simple,
  // 12s: 6s expanding/inhale, 6s contracting/exhale). This drives a matching
  // tone using performance.now(), which shares the same time origin as the
  // CSS animation timeline, so the audio stays in step with the visual.
  ...
  function initBreathPacers() {
    if (!$('.pacer-static')) return;
    const word = $('#pacer-word');
